...
class DAQ_Move_MaiTai(DAQ_Move_base):
    ...
    _epsilon = 0.2  # Wavelength setting precision in nm for move_done signal

    # Signals for UI updates, connecting the hardware controller to the custom UI
    ...
    pump_power_signal = Signal(float)
    diode1_current_signal = Signal(float)
    diode2_current_signal = Signal(float)
    ...
    def get_widget_and_signals(self):
        """
        Setup the UI widget and connect its signals. This is called by the DAQ_Move module
        when it wants to display the specific UI of the actuator.
        """
        min_wl = self.settings.child("bounds_group", "min_position").value()
        max_wl = self.settings.child("bounds_group", "max_position").value()
        self.ui = MaiTaiUI(min_wl=min_wl, max_wl=max_wl)
        ...
        self.ui.set_wavelength_signal.connect(self.set_wavelength_from_ui)

        # Connect controller status signals to UI update slots
        ...
        self.pump_power_signal.connect(self.ui.update_pump_power)
        self.diode1_current_signal.connect(self.ui.update_diode1_current)
        self.diode2_current_signal.connect(self.ui.update_diode2_current)
    ...
    def ini_stage(self, controller=None):
        """
        Initialize the controller and communication with the hardware.
        This method is called by the DAQ_Move main module.
        """
        self.ini_stage_init(old_controller=controller,
                            new_controller=MaiTaiController(
        ...
        info = f"MaiTai Laser connected on {self.settings.child('connection_group', 'serial_port').value()}"

        polling_interval = self.settings.child("polling_interval").value()
        self.status_timer.start(polling_interval)
        self.poll_status() # Initial status poll

        self.initialized_state.emit(True)
        return info, True

    def close(self):
        """Terminate the communication and stop timers."""
    ...
    def get_actuator_value(self):
        """
        Get the current value from the hardware. This method is polled by the DAQ_Move base class
        to check if the target position has been reached.
        """
        try:
            wavelength = self.controller.get_wavelength()
            return DataActuator(data=wavelength, units=self._controller_units)
        except Exception as e:
            self.emit_status(ThreadCommand("Update_Status", [f"Error reading wavelength: {e}", "error"]))
            return DataActuator(data=0, units=self._controller_units)

    def move_abs(self, position: DataActuator):
        """
        Move the actuator to an absolute target position.
        This method is called by the DAQ_Move module to start a move.
        """
        try:
            target_wavelength = self.check_bound(position.value())
            self.controller.set_wavelength(target_wavelength)
            self.target_position = DataActuator(data=target_wavelength, units=self._controller_units)
            self.emit_status(ThreadCommand("Update_Status", [f"Moving to {target_wavelength} nm", "log"]))
        except Exception as e:
            self.emit_status(ThreadCommand("Update_Status", [f"Error during move: {e}", "error"]))
            self.move_done()

    def stop_motion(self):
        """
        Stop the motion of the actuator. For the MaiTai, there is no specific stop command,
        so we simply signal that the current move operation is finished.
        """
        self.move_done()

    def poll_status(self):
        """Periodically query the laser for its status and emit signals to update the UI."""
        ...
        try:
            ...
            self.pump_power_signal.emit(self.controller.get_pump_power())
            self.diode1_current_signal.emit(self.controller.get_diode_current(1))
            self.diode2_current_signal.emit(self.controller.get_diode_current(2))
        except Exception as e:
            logger.warning(f"Status polling failed: {e}")

    # --- Methods to be called from the custom UI ---
    def set_wavelength_from_ui(self, wavelength: float):
        """Initiates a move from the custom UI."""
        self.move_abs(DataActuator(data=wavelength, units=self._controller_units))
